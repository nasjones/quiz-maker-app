...
export default function question(props) {
    ...
    let options = props.answers.map((option, index) => {
        ...
        if ((props.selected === he.decode(option))
            && (props.correct !== he.decode(option)))
            return (
                <div className='questLabel redOpt' key={index + 1}>
                    <input type='radio' name='option' className='quizOpt'
                        id={`opt${index + 1}`} value={he.decode(option)} disabled={true}
                    />
